...
import { myHomePageSelectedCity, myHomePageSelectedDate, myHomePageSelectedService, mySelectedDate, mySelectedServiceList, mySelectedSlot, mySpaPageSelectedCity, serviceSliceSelector } from "@/redux/service";
...
export default function Spas() {
  ...
  const { homeSelectedCity } = useSelector(serviceSliceSelector)
  ...
  const [loadingCounter, setLoadingCounter] = useState(0); // Track concurrent loading operations
  ...
  const [serviceDesc, setServiceDec] = useState(null)
  ...
  if(property1) {
    push(`${PATH_AUTH?.services}/${property1}`);
    return;
  }
  ...
  }, [currentPage, location, activeChips]);
  ...
  useEffect(() => {
    if (homeSelectedCity) {
      const selectedOption = cityDropDownData.find(
        (option) => option.value === homeSelectedCity.value
      );
      setSelectedCityOption(selectedOption || null);
      setSelectedCityOptionTemp(selectedOption || null)
      setSelectedTempCity(selectedOption || null)
      setSelectedTempMobileCity(selectedOption || null)
    }
  }, [homeSelectedCity, cityDropDownData]);
            ...
            <div className="sitback-banner-image-div">
              ...
              <div className="banner-content-wrapper spa-page-banner-content-div">
                <div className="banner-top-title-div">
                  <Container>
                    <h1>{t('spaPageText2')}</h1>
                    <p className="spa-page-banner-para-text">{t("spaPageText3")}</p>
                    <p className="breadcrumb-text bread-crumb-spa-text bread-crumb-spas-detail-wrapper">{t('homeText')} <span className="sign-text">&gt;</span> {t('spasText')}</p>
                  </Container>
                </div>
              </div>
            </div>
