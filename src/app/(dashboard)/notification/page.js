...
import { chatHandle, messageTabHandle, tabHandle } from "@/redux/messageTab";
...
import { NotificationLayoutWrapper } from "@/styles/pages/profile.style";
    ...
    if (window.io) {
      if (window?.location?.pathname.includes("notification")) {
        window.io.socket.on("serviceprovider", async (msg) => {
          if (msg.action == "new_booking_from_user" || msg.action == "new_review_from_user") {
            getNotification(1);
          }
          getMessage(1);
        });
              ...
              {activeTab === "notification" && (
                <div className="sitback-notification-list-wrapper">
                  ...
                  >
                   ...
                   <p className="notes-available-text">{t('noRecAvail')}</p> : null
                }
                  </InfiniteScroll>
                </div>
              )}
