...
const SuggestPastRequestModal = ({ show, handleClose,data,setItems,checkPendingAppointment }) => {
 ...
 const [slotLoading, setSlotLoading] = useState(true);
  ...
  today.setHours(0, 0, 0, 0); // Set time to 00:00:00 to match the 'T00:00:00' format
  ...
  useEffect(() => {
    ...
    if(appointmentType == "manual" && data) {
      getManualSlotData()
    }
          ...
          <SitBackModalBodyWrapper>
            ...
            <Form onSubmit={handleSubmit(onSubmitForm)} className="sit-req-form-wrapper">
              ...
              acceptData?.isReadyForApproval === true && acceptData?.slotListOutData.length === 0 ? (
                ...
                <>
                  {manualDataLoading ? (
                    <></> // If manualDataLoading is true, render nothing
                  ) : (
                    !spaCloseMsgManual ? (
                      <Button
                        type="submit"
                    ...
                    ) : null // If spaCloseMsgManual is false, render nothing
                  )}
                </>
              )
            }
...
export default SuggestPastRequestModal;
