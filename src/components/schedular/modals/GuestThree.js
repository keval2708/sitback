  ...
  useEffect(() => {
    ...
    if (schedulerResponse?.guestService3 && schedulerResponse?.guestService3?.value != '') {
      if (schedulerResponse?.guest3Detail) {
        ...
        setValue('guest3slots', `${schedulerResponse?.guest3Detail?.slots.slot_time} ${schedulerResponse?.guest3Detail?.slots?.time_type}`);
      } else {
        setValue('services', {
      ...
      <SchedulerModalLayoutWrapper>
        <Form onSubmit={handleSubmit(onSubmitForm)}>
          ...
          <LoadingButton
            ...
            loadinglabel={schedulerResponse?.guest == 3 ? `${t('confirmAndPay')}` : 'Continue to Guest #4'}
            isLoading={loading}
