...
const GuestOne = () => {
  ...
  const { schedulerData, schedulerResponse, serviceData, guestResponse } = useSelector(schedulerSliceSelector);
  ...
  useEffect(() => {
    if (schedulerResponse?.guestService1 && schedulerResponse?.guestService1?.value != '') {
      if (schedulerResponse?.guest1Detail) {
        ...
        setValue('guest1slots', `${schedulerResponse?.guest1Detail?.slots.slot_time} ${schedulerResponse?.guest1Detail?.slots?.time_type}`);
      } else {
      ...
      <SchedulerModalLayoutWrapper>
        <Form onSubmit={handleSubmit(onSubmitForm)}>
          ...
          <LoadingButton
            ...
            loadinglabel={schedulerResponse?.guest == 1 ? `${t('confirmAndPay')}` : "Continue to Guest #2"}
            isLoading={loading}
