  ...
  useEffect(() => {
    if (schedulerResponse?.guestService2 && schedulerResponse?.guestService2?.value != '') {
      if (schedulerResponse?.guest2Detail) {
        ...
        setValue('guest2slots', `${schedulerResponse?.guest2Detail?.slots.slot_time} ${schedulerResponse?.guest2Detail?.slots?.time_type}`);
      } else {
        setValue('services', {
      ...
      <SchedulerModalLayoutWrapper>
        <Form onSubmit={handleSubmit(onSubmitForm)}>
          ...
          <LoadingButton
            ...
            loadinglabel={schedulerResponse?.guest == 2 ? `${t('confirmAndPay')}` : "Continue to Guest #3"}
            isLoading={loading}
