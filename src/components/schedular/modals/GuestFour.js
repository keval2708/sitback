...
import { handleStep, manageGuestResponse, manageSchedulerResponse, schedulerSliceSelector } from "@/redux/scheduler";
...
const GuestOne = () => {
  ...
  const [employeeDataText, setEmployeeDataText] = useState('Please select service see available provider list');
  ...
  const { schedulerData, schedulerResponse, guestResponse, serviceData } = useSelector(schedulerSliceSelector);
  ...
  useEffect(() => {
    if (schedulerResponse?.guestService4 && schedulerResponse?.guestService4?.value != '') {
      if (schedulerResponse?.guest4Detail) {
        ...
        setValue('guest4slots', `${schedulerResponse?.guest4Detail?.slots.slot_time} ${schedulerResponse?.guest4Detail?.slots?.time_type}`);
      } else {
        setValue('services', {
    ...
    <>
      <SchedulerModalLayoutWrapper>
        <Form onSubmit={handleSubmit(onSubmitForm)}>
          ...
          />
          <Button isBorderBtn={true} type="reset" onClick={() => goBack()}>{t('goBack')}</Button>
        </Form>
      </SchedulerModalLayoutWrapper>
