...
const CustomCalender = ({
  ...
  useEffect(() => {
    if (window.io) {
      window.io.socket.on("serviceprovider", async (msg) => {
        if (msg.action == "new_booking_from_user" || msg.action == "auto_no_show_guest_booking_alert" || msg.action == "cancelBookingSpa" ||
          msg.action == "auto_no_show_user_booking_alert" || msg.action == "auto_start_service_user_booking_alert"
          || msg.action == "auto_start_service_guest_booking_alert" || msg.action == "compltedBySpa") {
          calenderBookingList();
        }
      });
    }
    ...
    if (currentView === "dayGridMonth") {
      ...
      if (upcomingCalenderData && upcomingCalenderData.length > 0) {
        upcomingCalenderData.forEach((item) => {
          ...
          //     // console.log("subEv?.sameDayEvents", subEv)
          ...
          //         : [evtObj];
          //     // console.log("cancel_event", cancel_event)
          ...
          //           : true,
          //     }));
          //     // console.log("11111 cancel_event", cancel_event)
          //     // return subEv;
          ...
          //       }));
          //   });
          //   // console.log("111111 clonedData", clonedData)
          //   // events = clonedData;
  ...
  function CustomTimeGrid(info) {
    ...
    return (
      <>
        <div className={info?.event?._def?.extendedProps?.slotDuration && info?.event?._def?.extendedProps?.slotDuration < 30 ? "fc-event-content-small" : "fc-event-content"}>
